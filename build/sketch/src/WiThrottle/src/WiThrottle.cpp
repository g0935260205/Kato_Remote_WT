#line 1 "C:\\Users\\96\\Documents\\Arduino\\Kato_Remote_WT\\src\\WiThrottle\\src\\WiThrottle.cpp"
/* -*- c++ -*-
 *
 * WiThrottle
 *
...
void WiThrottle::resetChangeFlags()
{
    clockChanged = false;
    heartbeatChanged = false;
}

void WiThrottle::connect(Stream *stream)
{
    init();
    this->stream = stream;
}

void WiThrottle::disconnect()
{
    this->stream = NULL;
}

void WiThrottle::setDeviceName(String deviceName)
{
    String command = "N" + deviceName;
    sendCommand(command);
}

void WiThrottle::setDeviceID(String deviceId)
{
    String command = "H" + deviceId;
    sendCommand(command);
}

bool WiThrottle::check()
{
    bool changed = false;
    resetChangeFlags();
...
void WiThrottle::sendCommand(String cmd)
{
    if (stream)
    {
...
bool WiThrottle::processLocomotiveAction(char *c, int len)
{
    ...
    //     // Serial.println(currentAddress.c_str());
    // }

    // Serial.println(c);
    for (int i = 0; i < 2; i++)
    {
        String addrCheck = AcurrentAddress[i] + PROPERTY_SEPARATOR;
        if (remainder.startsWith(addrCheck))
        {
            remainder.remove(0, addrCheck.length());
            currentAddress = AcurrentAddress[i];
            if (delegate)
            {
                delegate->CurrentAddr = AcurrentAddress[i];
            }
        }
    }
    ...
    //     remainder.remove(0, addrCheck.length());
    // }
    if (remainder.startsWith(allCheck))
...
bool WiThrottle::processCommand(char *c, int len)
{
    bool changed = false;
...
void WiThrottle::setCurrentFastTime(const String &s)
{
    int t = s.toInt();
    if (currentFastTime == 0.0)
    {
        console->print("set fast time to ");
        console->println(t);
    }
    else
    {
        console->print("updating fast time (should be ");
        console->print(t);
        console->print(" is ");
        console->print(currentFastTime);
        console->println(")");
#if defined ESP32 || defined ESP8266
...
bool WiThrottle::processFastTime(char *c, int len)
{
    ...
    if (p > 0)
    {
        ...
        console->print("set clock rate to ");
        console->println(currentFastTimeRate);
        changed = true;
        clockChanged = true;
    }
    else
...
bool WiThrottle::processHeartbeat(char *c, int len)
{
    bool changed = false;
    String s(c);
...
void WiThrottle::processProtocolVersion(char *c, int len)
{
    if (delegate && len > 0)
    {
...
void WiThrottle::processWebPort(char *c, int len)
{
    if (delegate && len > 0)
    {
...
void WiThrottle::processRosterList(char *c, int len)
{
    ...
    console->print("Entries in roster: ");
    console->println(entries);

    // if set, call the delegate method
    if (delegate)
        delegate->receivedRosterEntries(entries);
    ...
    for (int i = 0; i < entries; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            console->print(rosterSegmentDesc[j]);
            console->print(": ");
            console->println(segment);
        ...
        // if set, call the delegate method
        if (delegate)
            delegate->receivedRosterEntry(i, name, address, length);
...
void WiThrottle::processFunctionState(const String &functionData)
{
...
void WiThrottle::processSpeed(const String &speedData)
{
...
void WiThrottle::processSpeedSteps(const String &speedStepData)
{
...
void WiThrottle::processDirection(const String &directionStr)
{
...
void WiThrottle::processTrackPower(char *c, int len)
{
    if (delegate)
    {
        if (len > 0)
...
void WiThrottle::processAddRemove(char *c, int len)
{
    if (!delegate)
    {
...
void WiThrottle::processStealNeeded(char *c, int len)
{
    if (!delegate)
    {
        // If no one is listening, don't do the work to parse the string
...
void WiThrottle::requireHeartbeat(bool needed)
{
    if (needed)
    {
        sendCommand("*+");
...
bool WiThrottle::addLocomotive(String address)
{
    // Serial.println(address);
    bool ok = false;
    // Serial.println(address.startsWith("L"));
    if (address.startsWith("S") || address.startsWith("L"))
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (AcurrentAddress[i] == "S0")
            {
                AcurrentAddress[i] = address;
                if (delegate)
                {
                    delegate->AcurrentAddress[i] = AcurrentAddress[i];
                }
                // Serial.println(AcurrentAddress[i]);
                break;
            }
            if (i == 2)
            {
                AcurrentAddress[1] = address;
                if (delegate)
                {
                    delegate->AcurrentAddress[1] = AcurrentAddress[1];
                }
            }
        }
...
bool WiThrottle::stealLocomotive(String address)
{
    bool ok = false;
...
bool WiThrottle::releaseLocomotive(String address)
{
    // MT-*<;>r
    String cmd = "MT-";
    cmd.concat(address);
...
bool WiThrottle::setSpeed(int addr_index, int speed)
{
    ...
    if (speed != currentSpeed)
    {
        ...
        // TCFan add specific
        String cmd = "MTA";
        cmd.concat(delegate->AcurrentAddress[addr_index]);
        cmd.concat(PROPERTY_SEPARATOR);
        cmd.concat("V");
...
int WiThrottle::getSpeed()
{
    return currentSpeed;
}

bool WiThrottle::setDirection(Direction direction)
{
    if (!locomotiveSelected)
    {
        return false;
...
void WiThrottle::setFunction(int funcNum, bool pressed)
{
    if (!locomotiveSelected)
    {
        return;
